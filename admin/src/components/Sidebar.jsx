            ...
            <div key={section.label} className={cn(idx > 0 && 'mt-6')}>
              {!isCollapsed && (
                <div className="px-3 mb-2">
                  <h3 className="text-xs font-bold font-red-hat text-[#9CA3AF] uppercase tracking-widest">
                    {section.label}
                  </h3>
                </div>
              )}
              <div className="space-y-1">
                {section.items.map((item) => (
                  ...
                  >
                    ...
                    {!isCollapsed && <span className="flex-1 uppercase tracking-wider text-[11px]">{item.label}</span>}
                    {isActive(item.path) && (
                      <motion.div
