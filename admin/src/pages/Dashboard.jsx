...
const StatCard = ({ title, value, icon: Icon, accent, description, index }) => (
  ...
  >
    ...
    <div className="text-3xl font-bold font-space-mono text-[#1C1B1A] mb-1">
      {value ?? <span className="text-[#9CA3AF] text-xl font-red-hat">—</span>}
    </div>
    <div className="text-sm font-bold font-red-hat text-[#1C1B1A] mb-0.5 uppercase tracking-wide">{title}</div>
  ...
  >
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <div className="text-2xl font-bold font-space-mono text-[#1C1B1A] mb-1 uppercase">
            {count ?? <span className="text-[#9CA3AF] text-base font-red-hat">—</span>}
          </div>
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        padding: 12,
        cornerRadius: 8,
        titleFont: { family: "Red Hat Display", size: 12, weight: "600" },
        bodyFont: { family: "Red Hat Display", size: 11 },
      },
    },
    scales: {
      y: {
        beginAtZero: true,
        ticks: { stepSize: 1, precision: 0, color: "#9CA3AF", font: { family: "Red Hat Display", size: 11 } },
        grid: { color: "#F5F1E8" },
        border: { display: false },
      },
      x: {
        ticks: { color: "#9CA3AF", font: { family: "Red Hat Display", size: 11 } },
        grid: { display: false },
        border: { display: false },
      },
    },
    ...
    <div className="min-h-screen pb-12 px-4 bg-[#FAF8F4]">
      <div className="max-w-7xl mx-auto pt-8">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            <h1 className="text-4xl font-bold font-fraunces text-[#1C1B1A] mb-1">Dashboard</h1>
