...
const PropertyGridCard = ({ property, onRemove }) => (
  ...
  >
    {/* Clickable Area */}
    <Link to={`/update/${property._id}`} className="block">
      {/* Image */}
      <div className="relative h-48 overflow-hidden bg-[#F5F1E8]">
        {(() => {
          const imageUrl = Array.isArray(property.image) ? property.image[0] : (typeof property.image === 'string' ? property.image : null);
          return imageUrl ? (
            <img 
              src={imageUrl} 
              alt={property.title}
      ...
      <div className="p-4 pb-0">
        <h3 className="font-bold text-[#1C1B1A] text-base mb-1 line-clamp-1 group-hover:text-[#C5A059] transition-colors">{property.title}</h3>
...
const PropertyListRow = ({ property, onRemove }) => (
  ...
  >
    ...
    <Link to={`/update/${property._id}`} className="flex-1 flex items-center gap-4 min-w-0">
      {/* Thumbnail */}
      <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#F5F1E8] flex-shrink-0 group">
        {(() => {
          const imageUrl = Array.isArray(property.image) ? property.image[0] : (typeof property.image === 'string' ? property.image : null);
          return imageUrl ? (
            <img 
              ...
              onError={(e) => {
                e.target.onerror = null;
                e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center"><svg class="w-6 h-6 text-[#E6D5C3]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"></path><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"></path><path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2"></path><path d="M10 6h4"></path><path d="M10 10h4"></path><path d="M10 14h4"></path><path d="M10 18h4"></path></svg></div>';
              }}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <h3 className="font-semibold text-[#1C1B1A] text-sm truncate group-hover:text-[#C5A059] transition-colors">{property.title}</h3>
...
const PropertyListings = () => {
  ...
  const fetchProperties = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/api/admin/properties/all');
